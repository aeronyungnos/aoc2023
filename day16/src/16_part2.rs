...
fn main() {
    ...
    for i in 0..map_size {
        let count_unique_coordinates: i32 =
            find_coordinates(Direction::East, &Coordinates { y: i, x: 0 }, &map);
        count.push(count_unique_coordinates)
    }

    for i in 0..map_size {
        let count_unique_coordinates: i32 = find_coordinates(
            Direction::West,
            &Coordinates {
                y: i,
                x: map_size - 1,
    ...
    for i in 0..map_size {
        let count_unique_coordinates: i32 =
            find_coordinates(Direction::South, &Coordinates { y: 0, x: i }, &map);
        count.push(count_unique_coordinates)
    }
...
fn path(
    ...
    map: &Vec<&str>,
    visited: &mut Vec<(Coordinates, Direction)>,
) {
    if let Some(curr_spot) = map
        .get(coordinates.y)
        .and_then(|line| line.chars().nth(coordinates.x))
    {
        if !visited.contains(&(coordinates.clone(), direction)) {
            visited.push((coordinates.clone(), direction));
            match direction {
                Direction::North => match curr_spot {
                    '-' => {
                ...
                Direction::East => match curr_spot {
                    '|' => {
                        if let Some(result) = coordinates.y.checked_sub(1) {
                            path(
                                Direction::North,
                ...
                Direction::South => match curr_spot {
                    '-' => {
                        if let Some(result) = coordinates.x.checked_sub(1) {
                            path(
                                Direction::West,
                ...
                Direction::West => match curr_spot {
                    '|' => {
                        if let Some(result) = coordinates.y.checked_sub(1) {
                            path(
                                Direction::North,
